  ...
  <s-section :theme="theme" :bg="bg" class="section-double">
    <b-row class="no-gutters" :class="{ 'flex-row-reverse': reverse }">
      <b-col xl="4" md="5" class="content">
        <h2 v-if="content && content.title">{{ content.title }}</h2>
        <p v-if="content && content.description" class="lead text-muted">{{ content.description }}</p>
        <s-button v-if="content && content.link" :to="content.link.slug" variant="outline-primary" size="lg">{{ content.link.title }}</s-button>
      </b-col>
