                ...
                <form>
                  <div class="bg-body p-4 p-md-5 mb-4">
                    ...
                    <b-row class="mb-5">
                      ...
                      <!-- If Delivery -->
                      <template v-if="checkoutPrototype.orderType === 'DELIVERY'">
                        <b-col sm="6">
                          <b-form-group label="Delivery Area: *">
                            <ValidationProvider name="City" rules="required|min:2" v-slot="v">
                              <s-form-select v-model="checkoutPrototype.deliveryArea" :options="deliveryAreas" :optionAdapter="item => ({ label: item.city, id: item.zipCode, item })" name="deliverArea" :state="getInputState(v)" />
                            </ValidationProvider>
                          </b-form-group>
                        </b-col>
                        <b-col sm="6">
                          <b-form-group label="Zip Code: *">
                            <ValidationProvider name="ZipCode" rules="required|min:2" v-slot="v">
                              <b-form-input :value="checkoutPrototype.deliveryArea.zipCode || '-'" name="zipCode" :state="getInputState(v)" disabled />
                            </ValidationProvider>
                          </b-form-group>
                        </b-col>
                      </template>
                      <!-- If Pickup -->
                      <template v-else-if="checkoutPrototype.orderType === 'PICKUP'">
                        <b-col sm="6">
                          <b-form-group label="City: *">
                            <ValidationProvider name="City" rules="required|min:2" v-slot="v">
                              <b-form-input v-model="checkoutPrototype.city" name="city" :state="getInputState(v)" />
                            </ValidationProvider>
                          </b-form-group>
                        </b-col>
                      </template>
